...
gotoLabel  = Token.identifier lexer
...
blockFragment :: Parser (GotoLabel, Com)
blockFragment =
  do label <- gotoLabel
     reservedOp ":"
     stmt <- statement
...
blockEnd :: Parser [(GotoLabel, Com)]
blockEnd =
  do reserved "end"
     return $ []

blockFragments :: Parser [(GotoLabel, Com)]
blockFragments =      blockEnd
                 <|> (do fragment <- blockFragment
