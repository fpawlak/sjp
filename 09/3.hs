...
-- pierwszy przebieg
process1 :: Dec -> (EnvV, Store, [(EnvV, Procname, Varname, Com)]) -> (EnvV, Store, [(EnvV, Procname, Varname, Com)])
process1 Empty stuff = stuff
process1 (Let x a ds) (envV, sto, ps) = process1 ds (envV', sto', ps)
  where (envV', sto') = declareVariable envV sto x v
        v = aeval a envV sto
process1 (Proc procname param c ds) (envV, sto, ps) = process1 ds (envV, sto, ((envV, procname, param, c):ps))

-- drugi przebieg (po procedurach)
process2 :: EnvP -> [(EnvV, Procname, Varname, Com)] -> EnvP
process2 envP ps = updateEnvP (fix f)
  where updateEnvP procList = foldl putProc' envP $ zip names procList        
        names = map (\(_, procname, _, _) -> procname) ps
        putProc' e = uncurry $ putProc e
        f g = map makeProcedure ps
          where makeProcedure (envV, procname, param, c) arg sto = sem c envV' envP' sto'
                  where (envV', sto') = declareVariable envV sto param arg
                envP' = updateEnvP g        


process :: Dec -> (EnvV, EnvP, Store) -> (EnvV, EnvP, Store)
process ds (envV, envP, sto) = (envV', envP', sto')
  where (envV', sto', ps) = process1 ds (envV, sto, [])
        envP' = process2 envP ps
